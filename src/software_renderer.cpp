...
namespace CMU462 {
    ...
    void SoftwareRendererImp::fill_sample(int sx, int sy, const Color& c) {
        uint8_t new_r, new_g, new_b, new_a;
        uint8_t old_r, old_g, old_b, old_a;

        old_r = sample_buffer[4L * ((size_t)sx + (size_t)sy * w)];
        old_g = sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 1];
        old_b = sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 2];
        old_a = sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 3];

        if (old_r == 255 && old_g == 255 && old_b == 255 && old_a == 255) {
        new_r = (uint8_t)(c.r * 255);
        new_g = (uint8_t)(c.g * 255);
        new_b = (uint8_t)(c.b * 255);
        new_a = (uint8_t)(c.a * 255);
        }

        
        else {
            new_r = (uint8_t)(((1 - c.a) * ((float)old_r / 255) + (c.r)) * 255);
            new_g = (uint8_t)(((1 - c.a) * ((float)old_g / 255) + (c.g)) * 255);
            new_b = (uint8_t)(((1 - c.a) * ((float)old_b / 255) + (c.b)) * 255);
            new_a = (uint8_t)((1 - (1 - c.a) * (1 - (float)old_a / 255)) * 255);
        }

        sample_buffer[4L * ((size_t)sx + (size_t)sy * w)] = new_r;
        sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 1] = new_g;
        sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 2] = new_b;
        sample_buffer[4L * ((size_t)sx + (size_t)sy * w) + 3] = new_a;
    }
    ...
    void SoftwareRendererImp::resolve(void) {
        ...
        for (size_t x = 0; x < target_w; x++) {
            for (size_t y = 0; y < target_h; y++) {
                ...
                r = r / (float)((rate * rate) * 255);
                g = g / (float)((rate * rate) * 255);
                b = b / (float)((rate * rate) * 255);
                a = a / (float)((rate * rate) * 255);
